...
namespace LidarTool
{
    ...
    public class LidarFunc
    {
        ...
        //Lodar資料
        public static byte[] LidarBuffer;
        public static double[] VerticalAngle;
        public static double[] CosVerticalAngle;
        public static double[] SinVerticalAngle;

        //720分割
        public static LidarData rLidarData;

        //360分割
        public static LidarData r_LidarData;
        public static int LidarUpdateCount = 0;

        //存放Lidar-XYZ座標的List
        public static List<float> LidarWriteBufferIndex = new List<float>();
        public static List<float> LidarWriteBufferX = new List<float>();
        public static List<float> LidarWriteBufferY = new List<float>();
        public static List<float> LidarWriteBufferZ = new List<float>();


        //表是否開始記錄Lidar資料
        public static bool isLidarDataRecode = false;

        //Lidar資料總數
        public static int TotalLidarCount = 0;

        //lidar開始收資料
        public static bool isLidarStart = false;

        //記錄SharpGL初始游標位置
        public static Point _BakMousePosition = new Point(0, 0);

        //讀資料的StreamReader
        public static StreamReader LidarReader;

        //讀檔案資料的Thread
        public static Thread ReadFileThread = null;

        //紀錄是否讀到檔案底部
        public static bool isEOF = false;

        #endregion    
        Ini Lidar_Ini = new Ini();
        public static double Lidar_Accuracy;
        public static double Lidar_Segmentation;

        public void Init()
        {
            Lidar_Ini.Read_ini_Cfg3();
            LidarFunc.Lidar_Accuracy = 1 / Lidar_Ini.Lidar_Accuracy;
            //Lidar_Accuracy = 1 / Lidar_Ini.Lidar_Accuracy;
            LidarFunc.Lidar_Segmentation = LidarFunc.Lidar_Accuracy * 360;
            ...
            /*rLidarData.X = new float[(int)(LidarFunc.Lidar_Segmentation * 32)];
            rLidarData.Y = new float[(int)(LidarFunc.Lidar_Segmentation * 32)];
            rLidarData.Z = new float[(int)(LidarFunc.Lidar_Segmentation * 32)];*/

            r_LidarData = new LidarData();
            r_LidarData.X = new float[11520];
            r_LidarData.Y = new float[11520];
            r_LidarData.Z = new float[11520];
            /*r_LidarData.X = new float[(int)((LidarFunc.Lidar_Segmentation * 32) / 2)];
            r_LidarData.Y = new float[(int)((LidarFunc.Lidar_Segmentation * 32) / 2)];
            r_LidarData.Z = new float[(int)((LidarFunc.Lidar_Segmentation * 32) / 2)];*/
        }

        public static void StartLidarUDP()
        {
            //UDP光達封包接收
            try
            {
        ...
        public static void LidarDataOut()
        {
            //輸出Lidar資料
            int LidarCount = 0;
        ...
        public static void ReadLidarDataFuncThread()
        {
            while (!isEOF)
            {
                //解析檔案裡的每行資料
        ...
        public static void LoadLidarVariable()
        {
            //讀取Lidar固定變數
            //Lidar垂直固定角度
